...
export class CountryInfoService {
  ...
  public getAllCountries() {
    return this.resourceService
      .getFullRequest<Country[]>('AvailableCountries')
      .pipe(
        map(resp => {
  ...
  public getCountryInfo(countryCode: string) {
    return this.resourceService
      .getFullRequest<CountryInfo>(`CountryInfo/${countryCode}`)
      .pipe(
        map(resp => {
  ...
  public getNextHolidayInfo(countryCode: string) {
    return this.resourceService
      .getFullRequest<CountryHolidayInfo[]>(`NextPublicHolidays/${countryCode}`)
      .pipe(
        map(resp => {
          if (resp.body != null) {
